...
const addMembers = async (req, res) => {
  try {
    if (!req.body.members) {
      res
        .status(200)
        .send({ success: false, msg: "Please select any one members!" });
...
const shareGroup = async (req, res) => {
  try {
    ...
    if (!groupData) { res.render("error", { message: "404 not found!" });
      ...
      const isJoined = await Member.find({ group_id: req.params.id, user_id: req.session.user._id }).count();
...
const groupChats = async (req,res)=>{ // error
  try {
